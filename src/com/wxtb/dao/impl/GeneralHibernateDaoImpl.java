...
import com.wxtb.dao.GeneralHibernateDao;
import com.wxtb.dao.PageResults;
import com.wxtb.dao.RowMapper;
 ...
 * }
 * </pre>
 * @ClassName: GeneralHibernateDaoImpl
 * @Description: GeneralHibernateDao实现
 * @author 昂军文
 * @date 2015年1月4日
 */
@Repository
@SuppressWarnings({ "rawtypes", "unchecked" })
public class GeneralHibernateDaoImpl<T, ID extends Serializable> implements GeneralHibernateDao<T, ID> {
	...
	public T findUniqueBy(String name, Object value) {
		Assert.hasText(name);
		return (T) getCriteria(getEntityClass(), Restrictions.eq(name, value)).uniqueResult();
	}
...
//		crit.setCacheRegion("query.shorttime_5");
		return crit.list();
	}
	...
	@Override
	public List<T> getInfoByPaging(Class<T> entityClass, int firstPosition,
			int sumItem) {
		Criteria crit = getSession().createCriteria(entityClass);
		crit.setFirstResult(firstPosition);
		crit.setMaxResults(sumItem);
